        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 p-4 border rounded-lg bg-slate-50">
            ...
            {turnstileState.error && (
              <div className="text-sm text-red-600 text-center">{turnstileState.error}</div>
            )}
          ...
          {executionStatus.isExecuting && (
            <div className="space-y-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-blue-800">Executing Test Cases...</span>
          ...
          <div className="flex gap-2">
            ...
            {executionStatus.isExecuting && (
              <Button onClick={stopExecution} variant="destructive" size="icon">
        ...
        <div className="mt-8 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {executionStatus.isComplete && (
              <span className="text-sm text-green-600 font-medium">✓ All test cases completed</span>
            )}
          </div>
          <div className="space-y-4">
            {Array.from(
              { length: Math.max(results.length, executionStatus.totalTestCases || 0) },
              (_, i) => {
                const result = results[i];
                const isExecuting =
                  executionStatus.isExecuting && i === executionStatus.currentTestCase && !result;
                const isPending = i >= executionStatus.currentTestCase && !result;

                return (
                  <div
                    key={
                      result?.testCaseIndex !== undefined
                        ? `result-${result.testCaseIndex}`
                        : `pending-${i}`
                    }
